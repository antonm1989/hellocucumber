...
const { Builder, By, Capabilities, Key, Browser, browser } = require('selenium-webdriver');
...
Then('I should see Home page', { timeout: 60 * 1000 }, async function () {
    await driver.wait(until.elementLocated(By.css(locators.footerSelector), 10000));
...
Then('I should see Authorization page', async function () {
    await driver.wait(until.elementLocated(By.css(locators.authPage), 10000));
...
Then('I should see entered password', async function () {
    await driver.wait(until.elementLocated(By.xpath(locators.enteredPassword), 10000));
...
Then('I should see User button', async function () {
    // await driver.sleep(5000);
    await driver.wait(until.elementLocated(By.xpath(locators.loggedInUserIcon), 10000));
    res = await driver.findElement(By.xpath(locators.loggedInUserIcon)).isDisplayed().then(value => { return true }, reason => { return false });
    assert.equal(res, true, "Logged in user icon is not displayed");
    let element = await driver.findElement(By.xpath(locators.loggedInUserIcon)).getAttribute("class");
    assert.equal(element, "ssls-toolbar__btn-text", "Dropdown is not present");
});
